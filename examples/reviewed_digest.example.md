Subject: Weekly journal digest for 2026-03-23 to 2026-03-29

## Email Summary

This week has a manageable set of papers with a few especially strong pieces in political communication, online influence, and causal inference. The email body should stay short, so the highlights are below and the full curated digest is reserved for the attached PDF.

## Collection Snapshot

- Total collected candidates: 25
- New This Week candidates: 12
- Previous Week Catch-Up candidates: 8
- Late Additions candidates: 5
- Curated digest below: 4 new, 2 catch-up, 1 late addition
- Note: These counts are from the full collected set before manual filtering.

## Highlights

- **Example article title**
  Journal: Journal of Communication
  Published: 2026-03-27
  Why it matters: It offers a clean example of the kind of paper that deserves one of the limited in-email highlight slots.
  Link: https://doi.org/example

- **Example catch-up highlight**
  Journal: Political Analysis
  Published: 2026-03-22
  Why it matters: It is a good example of a notable paper that can be highlighted even if it sits outside the current-week section.
  Link: https://doi.org/example-2

## Full Curated Digest

### New This Week

#### Journal of Communication
- **Example article title**
  Published: 2026-03-27
  Authors: Example Author; Example Collaborator
  Affiliations: Northwestern University; COMAP Lab
  DOI: 10.1000/example
  Link: https://doi.org/example
  Abstract: Example abstract text.

### Previous Week Catch-Up

#### Journal of Computer-Mediated Communication
- **Example catch-up title**
  Published: 2026-03-22
  Authors: Example Author
  DOI: 10.1000/example-2
  Link: https://doi.org/example-2
  Abstract: Example catch-up abstract.

### Late Additions

#### Political Analysis
- **Example late discovery**
  Published: 2026-03-15
  Authors: Example Author
  Affiliations: Northwestern University
  DOI: 10.1000/example-3
  Link: https://doi.org/example-3
  Abstract: Example late-addition abstract.
