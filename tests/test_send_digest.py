...
""",
            ...
            self.assertNotIn("Collection Snapshot", sent[0][3])
            self.assertNotIn("Full Curated Digest", sent[0][3])
    ...
    def test_send_digest_can_include_full_curated_digest_in_html_body(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir)
            config_path = repo / "sources.yaml"
            config_path.write_text(
...
- **Important article**
  ...
  Link: https://doi.org/example
  Abstract: Example abstract for the HTML body.
""",
                encoding="utf-8",
            )
            sent = []
